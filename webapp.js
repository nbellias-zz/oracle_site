...
function handleRequest(request, response, pool) {
    ...
    if (bathKwd == 'favicon.ico') {
        htmlFooter(response);
        return;
    }

    if (bathKwd != parseInt(bathKwd)) {
        handleError(
            response,
            'URL path "' + bathKwd + '" is not an integer.  Try http://localhost:' + httpPort + '/12',
            null
        );
...
🎆
        ...
        connection.execute(
            `SELECT bath_kwd, bath_syn, bath_lek
       FROM dm_bathmos
       WHERE bath_kwd = :id`, [bathKwd], // bind variable value
            function (err, result) {
                if (err) {
                ...
                displayResults(response, result, bathKwd);
...
function displayResults(response, result, bathkwd) {
    response.write("<h2>" + "Βαθμός ΣΞ " + bathkwd + "</h2>");
